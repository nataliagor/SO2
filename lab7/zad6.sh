...
#grep -oE '\\includegraphics(\[[^]]*\])?\{[^}]+\}' dodatkowe/slajdy.tex | sed 's/\\includegraphics(\[[^]]*\])?{//g; s/}//g' | sort -u


grep -oE '\\includegraphics(\[[^]]*\])?\{[^}]+\}' dodatkowe/slajdy.tex | sed 's/\\includegraphics//g' | sed 's/\[[^\{]*\{//g' | sort -u
