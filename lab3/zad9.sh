...
function szukanie_pliku {
	for podkatalog in "$1"/*; do
		if [ -f "${podkatalog}" ] && [ "$(basename "${podkatalog}")" = "$3" ]; then
	
			dobra_sciezka="$(realpath --relative-to="$(dirname "${2}")" "${podkatalog}")"
			echo ""$(basename "$2")": "${dobra_sciezka}""
	
		elif [ -d "${podkatalog}" ]; then
	
			szukanie_pliku "${podkatalog}" "$2" "$3"
...
for dowiazanie in dane/* dane/*/*; do
	if [ -h "${dowiazanie}" ] && [ ! -f "${dowiazanie}" ]; then

		nazwa_pliku="$(basename "${dowiazanie}")"
		katalog="dane"
		
#		echo "$(basename "${dowiazanie}") : "${nazwa_pliku}""	
		szukanie_pliku "${katalog}" "${dowiazanie}" "${nazwa_pliku}"		
	fi 
done
