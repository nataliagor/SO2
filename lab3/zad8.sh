...
for plik in dane/elements/*; do
        if [ -f "${plik}" ] && [ ! -h "${plik}" ] && [ ! -e "dane/pierwiastki/$(basename "${plik}")" ] && [ ! -h "dane/pierwiastki/$(basename "${plik}")" ]; then
		echo "P:"$(basename "${plik}")""
        fi
done


for plik in dane/pierwiastki/*; do
        if [ -f "${plik}" ] && [ ! -h "${plik}" ] && [ ! -e "dane/elements/$(basename "${plik}")" ] && [ ! -e "dane/elements/$(basename "${plik}")" ]; then
		echo "L:"$(basename "${plik}")""
        fi
done
